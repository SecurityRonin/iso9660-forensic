...
| MDF/MDS (Alcohol 120%) | ✅ (v0.3) | medium | `mds` descriptor parser; CLI windows the `.mdf` data track via `OffsetReader`. **Validated against a real Alcohol MDS** (Aaru-generated from our own ISO) ✅ — which corrected the `TrackMode` mapping: real Alcohol uses `0xA9`–`0xED` (Mode 1 = `0xAA`), not libmirage's `0x00`–`0x07` (those are the low-3-bits fallback). The earlier struct-only mapping mislabeled Mode 1 as Mode 2 |
