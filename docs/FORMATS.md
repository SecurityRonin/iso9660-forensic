...
| Apple HFS+/HFSX hybrid | ✅ (v0.3) | medium | `hfs` module (the `hfsplus-forensic` crate): volume-header geometry **plus full catalog B-tree** — `list_root`, `list_dir`, recursive `walk`, and `read_file` (data-fork extraction). CLI `hfs` command lists root / `-R` whole tree / extracts by path. **Validated against real `hdiutil` volumes** ✅ (header, populated, and nested 800 KB volumes; 8 tests). Out of scope: journal replay, resource forks, decmpfs, multi-extent catalog, classic (non-plus) HFS |
