...
| CDRDAO TOC/BIN | ✅ (v0.3) | low | `toc` module parses the `.toc` (disc type, `TRACK` modes, `DATAFILE`/`AUDIOFILE` offset + length); CLI windows the data file via `OffsetReader` to browse, and `tracks` shows the TOC. **Validated against a real Aaru-generated TOC** (from our own ISO) — single `MODE1_RAW` track, 188 sectors, offset 0, with an end-to-end `ls` round-trip ✅ |
