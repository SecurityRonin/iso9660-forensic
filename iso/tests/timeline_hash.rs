...
fn iso_with_file(content: &[u8]) -> Vec<u8> {
    ...
    d[78..82].copy_from_slice(&sz.to_le_bytes());
    d[82..86].copy_from_slice(&sz.to_be_bytes());
    d[100] = 4;
    d[101..105].copy_from_slice(b"FILE");
    // file data
...
fn hashlist_result_sorted_by_path() {
    ...
    d[70..74].copy_from_slice(&19u32.to_le_bytes());
    d[74..78].copy_from_slice(&19u32.to_be_bytes());
    d[78..82].copy_from_slice(&1u32.to_le_bytes());
    d[82..86].copy_from_slice(&1u32.to_be_bytes());
    d[100] = 5;
    d[101..106].copy_from_slice(b"ZFILE");
    // "AFILE": name_len=5 odd -> rec_len=38
    d[106] = 38;
    d[108..112].copy_from_slice(&20u32.to_le_bytes());
    d[112..116].copy_from_slice(&20u32.to_be_bytes());
    d[116..120].copy_from_slice(&1u32.to_le_bytes());
    d[120..124].copy_from_slice(&1u32.to_be_bytes());
    d[138] = 5;
    d[139..144].copy_from_slice(b"AFILE");
    img[19 * S] = b'Z';
    img[20 * S] = b'A';
    let mut reader = IsoReader::open(Cursor::new(img)).unwrap();
    let h = reader.hashlist().unwrap();
...
fn hashlist_sha256_is_64_hex_chars() {
    ...
    assert!(
        h[0].sha256_hex.chars().all(|c| c.is_ascii_hexdigit()),
