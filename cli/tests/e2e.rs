...
fn make_phantom_iso() -> Vec<u8> {
    const SS: usize = 2048;
    let mut img = vec![0u8; 22 * SS];
    let p = &mut img[16 * SS..17 * SS];
    p[0] = 0x01;
    ...
    p[132..136].copy_from_slice(&22u32.to_le_bytes());
    ...
    p[181] = 0x02;
    p[188] = 1;
    let t = &mut img[17 * SS..18 * SS];
    t[0] = 0xFF;
    t[1..6].copy_from_slice(b"CD001");
    t[6] = 0x01;
    let pt = &mut img[SS..2 * SS];
    pt[0] = 1;
    ...
    let mut off = 18 * SS;
    off += phantom_dir_rec(&mut img, off, 18, 2048, true, &[0x00]);
    phantom_dir_rec(&mut img, off, 18, 2048, true, &[0x01]);
    let mut off = 20 * SS;
    off += phantom_dir_rec(&mut img, off, 20, 2048, true, &[0x00]);
    off += phantom_dir_rec(&mut img, off, 18, 2048, true, &[0x01]);
    phantom_dir_rec(&mut img, off, 21, 5, false, b"GHOST.TXT");
    img[21 * SS..21 * SS + 5].copy_from_slice(b"ghost");
    img
}

#[test]
fn forensic_recover_finds_lost_file() {
    let dir = tempfile::tempdir().unwrap();
    let p = dir.path().join("lost.iso");
    std::fs::write(&p, make_phantom_iso()).unwrap();
    bin()
        .args(["forensic", "recover", p.to_str().unwrap()])
        .assert()
        .success()
